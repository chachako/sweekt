...
fun View.drawForeground(block: DrawableBuilder.() -> Unit) {
  if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.M) {
    foreground = DrawableBuilder().apply(block).build()
  }
}

fun FrameLayout.drawForeground(block: DrawableBuilder.() -> Unit) {
  foreground = DrawableBuilder().apply(block).build()
}

@DrawableBuilderMarker
class DrawableBuilder {
  ...
  // <corner>
  fun cornerRadius(cornerRadius: Number) = apply { properties.cornerRadius = cornerRadius.float }
  fun topLeftRadius(topLeftRadius: Number) = apply { properties.topLeftRadius = topLeftRadius.float }
  fun topRightRadius(topRightRadius: Number) = apply { properties.topRightRadius = topRightRadius.float }
  fun bottomRightRadius(bottomRightRadius: Number) = apply { properties.bottomRightRadius = bottomRightRadius.float }
  fun bottomLeftRadius(bottomLeftRadius: Number) = apply { properties.bottomLeftRadius = bottomLeftRadius.float }
  ...
  fun gradientRadius(radius: Float, type: Int) = apply {
  ...
  // <solid>
  fun solid(solidColor: Int) = apply { properties.solidColor = solidColor }
  private var solidColorPressed: Int? = null
  fun solidPressed(color: Int?) = apply { solidColorPressed = color }
  private var solidColorPressedWhenRippleUnsupported: Int? = null
  fun solidPressedWhenRippleUnsupported(color: Int?) = apply { solidColorPressedWhenRippleUnsupported = color }
  private var solidColorDisabled: Int? = null
  fun solidDisabled(color: Int?) = apply { solidColorDisabled = color }
  private var solidColorSelected: Int? = null
  fun solidSelected(color: Int?) = apply { solidColorSelected = color }
  fun solidStateList(colorStateList: ColorStateList) = apply { properties.solidColorStateList = colorStateList }
  ...
  fun degrees(fromDegrees: Float, toDegrees: Float) = apply {
    fromDegrees(fromDegrees).toDegrees(
      toDegrees
    )
  }

  fun degrees(degrees: Float) = apply { fromDegrees(degrees).toDegrees(degrees) }
  fun rotate(fromDegrees: Float, toDegrees: Float) = apply {
    rotate().fromDegrees(fromDegrees)
      .toDegrees(toDegrees)
  }
  ...
  fun scale(scaleWidth: Float, scaleHeight: Float) = apply {
  ...
  // <ripple>
  fun ripple(color: Int) = apply {
    properties.useRipple = true
    properties.rippleColor = color
  }

  fun rippleStateList(colorStateList: ColorStateList) = apply {
    properties.useRipple = true
    properties.rippleColorStateList = colorStateList
  }
  ...
  fun build(): Drawable {
    ...
    if (shouldFallbackRipple()) {
      if (solidColorPressedWhenRippleUnsupported != null) {
        solidPressed(solidColorPressedWhenRippleUnsupported)
      } else {
        solidPressed(properties.rippleColor)
      }
    }
