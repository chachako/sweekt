...
fun Project.importJars(
  ...
  FileUtils.getAllFiles(File(fromDir)).forEach {
    if (it.name.endsWith(SdkConstants.DOT_JAR) && !implemented.contains(it.parent)) {
      dependencyHandler.add(
        configurationName,
...
fun Project.importFlutterStub(dependencyHandler: DependencyHandler) = dependencyHandler.add(
  ...
  fileTree(
    mapOf(
      "dir" to "$marsLocalInternalPath.classpath",
