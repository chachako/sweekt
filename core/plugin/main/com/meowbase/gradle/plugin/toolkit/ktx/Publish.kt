...
fun Project.publishArchive(
  ...
  if (withDokka) {
    ...
    ) project.plugins.apply(PluginDokka)

    plugins.withId(PluginDokka) {
  ...
  afterEvaluate {
    ...
    MavenPublishConfigurer(this, pom).apply {
      if (plugins.hasPlugin("org.jetbrains.kotlin.multiplatform")) {
        configureKotlinMppProject(withJavadoc)
      } else if (plugins.hasPlugin("java-gradle-plugin")) {
        configureGradlePluginProject(withJavadoc)
      } else if (plugins.hasPlugin("com.android.library")) {
        configureAndroidArtifacts(withJavadoc)
...
@InternalMeowbaseApi
fun Project.publishMeowbaseArchiveToLocal(
  version: String = "1.0",
  group: String = Meowbase.group,
  artifact: String = project.name,
  withJavadoc: Boolean = false,
  withDokka: Boolean = false
) = publishArchive(
