...
fun DependencyHandler.apiMeowbaseLibraries(vararg artifacts: String) {
  artifacts.forEach { add("api", meowbaseLibrary(it)) }
}

fun DependencyHandler.testApiMeowbaseLibraries(vararg artifacts: String) {
  artifacts.forEach { add("testApi", meowbaseLibrary(it)) }
}