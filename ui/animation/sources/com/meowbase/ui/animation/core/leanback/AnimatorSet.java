...
public final class AnimatorSet extends Animator implements AnimationHandler.AnimationFrameCallback {
    ...
    // fixes their side.
    private AnimatorListenerAdapter mNoOpListener = new AnimatorListenerAdapter() {
        @Override
    ...
    public void cancel() {
        ...
        if (isStarted()) {
            ArrayList<android.animation.Animator.AnimatorListener> tmpListeners = null;
            if (mListeners != null) {
                tmpListeners = (ArrayList<android.animation.Animator.AnimatorListener>) mListeners.clone();
    ...
    private void start(boolean inReverse, boolean selfPulse) {
        ...
        if (mListeners != null) {
            @SuppressWarnings("unchecked")
            ArrayList<android.animation.Animator.AnimatorListener> tmpListeners =
                    (ArrayList<android.animation.Animator.AnimatorListener>) mListeners.clone();
            int numListeners = tmpListeners.size();
            for (int i = 0; i < numListeners; ++i) {
                android.animation.Animator.AnimatorListener listener = tmpListeners.get(i);
                if (listener instanceof AnimatorListener) {
                    ((AnimatorListener) listener).onAnimationStart(this, inReverse);
                } else {
    ...
    private void addNoOpListener() {
        for (int i = 1; i < mNodes.size(); i++) {
            mNodes.get(i).mAnimation.addListener(mNoOpListener);
        }
    }

    private void removeNoOpListener() {
        for (int i = 1; i < mNodes.size(); i++) {
            mNodes.get(i).mAnimation.removeListener(mNoOpListener);
        }
    }
    ...
    private void endAnimation() {
        ...
        if (mListeners != null) {
            ...
            for (int i = 0; i < numListeners; ++i) {
                android.animation.Animator.AnimatorListener listener = tmpListeners.get(i);
                if (listener instanceof AnimatorListener) {
                    ((AnimatorListener) listener).onAnimationEnd(this, mReversing);
                } else {
    ...
    public AnimatorSet clone() {
        ...
        anim.mNoOpListener = new AnimatorListenerAdapter() {
            @Override
            public void onAnimationEnd(@NonNull android.animation.Animator animation) {
        ...
        for (int n = 0; n < nodeCount; n++) {
            ...
            nodeClone.mAnimation.removeListener(mNoOpListener);
