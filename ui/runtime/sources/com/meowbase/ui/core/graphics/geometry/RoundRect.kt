...
data class RoundRect(
  ...
  private fun scaledRadiiRect(): RoundRect = _scaledRadiiRect ?: run {
    ...
    RoundRect(
      ...
      right = right * scale,
      bottom = bottom * scale,
      topLeftCornerRadius = CornerRadius(topLeftCornerRadius.x * scale, topLeftCornerRadius.y * scale),
      topRightCornerRadius = CornerRadius(topRightCornerRadius.x * scale, topRightCornerRadius.y * scale),
      bottomRightCornerRadius = CornerRadius(bottomRightCornerRadius.x * scale, bottomRightCornerRadius.y * scale),
      bottomLeftCornerRadius = CornerRadius(bottomLeftCornerRadius.x * scale, bottomLeftCornerRadius.y * scale)
    )
  }.also {
...
val RoundRect.boundingRect: RectF get() = RectF(left, top, right, bottom)
 ...
 */
val RoundRect.safeInnerRect: RectF
  get() {
...
val RoundRect.isRect
  get(): Boolean = (topLeftCornerRadius.x == 0.0f || topLeftCornerRadius.y == 0.0f) &&
 ...
 */
val RoundRect.hasSameCornerRadius: Boolean
  get() {
